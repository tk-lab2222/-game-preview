...
function render269(){evaluate269();const host=document.getElementById('missionHost203');if(!host)return;const st=state269(),s=short269(),sid='g'+generation269()+':'+s.id,done=!!st.completed[sid];host.innerHTML=`<div class="missionBoard269"><div class="missionTitle269"><div><small>BLOODLINE MISSIONS</small><b>🧬 血統ミッション</b></div><span>第${generation269()}世代</span></div><div class="shortMission269 ${done?'done269':''}"><small>今世代の目標</small><b>${done?'✓ ':''}${s.title}</b><p>${s.desc}</p><em>${done?'✓ 達成 / 報酬 🏆+1 受取済':'報酬：🏆+1 ・ 世代を越える前に狙おう'}</em></div><div class="longTitle269">長期目標 <small>世代をまたいで進行</small></div><div class="longList269">${LONG269.map(m=>{const d=!!st.completed[m.id];return `<div class="${d?'done269':''}"><span>${d?'✓':'◇'}</span><p><b>${m.title}</b><small>${m.desc}</small></p><em>${d?'✓ 達成 / 報酬 🏆+3 受取済':`報酬：🏆+3 ・ ${progress269(m)}`}</em></div>`}).join('')}</div><div class="missionNote269">大会で強い個体だけでなく、次世代へ残す価値のある血統を作るための目標です。</div></div>`}
function late269(){render269();[80,220,500].forEach(ms=>setTimeout(render269,ms))}
window.addEventListener('click',e=>{if(e.target?.closest?.('.tab[data-v="mission203"],#adopt,#doTrain263,#run,#next,#next217,#annualNext233'))setTimeout(late269,0)},true);
