...
function render269(){evaluate269();const host=document.getElementById('missionHost203');if(!host)return;let board=document.getElementById('bloodlineMission269');if(!board){board=document.createElement('div');board.id='bloodlineMission269';host.prepend(board)}else if(board.parentElement!==host)host.prepend(board);const st=state269(),s=short269(),sid='g'+generation269()+':'+s.id,done=!!st.completed[sid];board.innerHTML=`<div class="missionBoard269"><div class="missionTitle269"><div><small>BLOODLINE MISSIONS</small><b>🧬 血統ミッション</b></div><span>第${generation269()}世代</span></div><div class="missionBonus269"><div><small>MISSION BONUS</small><b>ネスト達成ボーナス</b></div><strong>全能力 +${bonus269()}</strong><em>達成 ${completedCount269()}件 × +5 / 現役3体へ常時反映</em></div><div class="shortMission269 ${done?'done269':''}"><small>今世代の目標</small><b>${done?'✓ ':''}${s.title}</b><p>${s.desc}</p><em>${done?'✓ 達成 / 全能力+5 反映済':'報酬：全能力+5 ・ 世代を越える前に狙おう'}</em></div><div class="longTitle269">長期目標 <small>世代をまたいで進行</small></div><div class="longList269">${LONG269.map(m=>{const d=!!st.completed[m.id];return `<div class="${d?'done269':''}"><span>${d?'✓':'◇'}</span><p><b>${m.title}</b><small>${m.desc}</small></p><em>${d?'✓ 達成 / 全能力+5 反映済':`報酬：全能力+5 ・ ${progress269(m)}`}</em></div>`}).join('')}</div><div class="missionNote269">大会で強い個体だけでなく、次世代へ残す価値のある血統を作るための目標です。</div></div>`}
