...
function render322(){
  ...
  box.innerHTML='<div class="missionHero322"><div><small>NEST MISSION LAB</small><h3>🎯 階級ミッション</h3><p>階級が上がるほど新しい目標が開放。能力報酬は「ネスト基礎値」として今後の育成3体にも引き継がれます。</p></div><strong>'+done+'/'+M322.length+'</strong></div>'+
    ...
    LEAGUES322.map((name,tier)=>{
      const unlocked=rank>=tier,missions=M322.filter(m=>m.tier===tier),count=missions.filter(m=>S.missionClaimed322[m.id]).length;
      return '<section class="missionTier322 '+(!unlocked?'tierLocked322':'')+'"><header><div><small>'+ICONS322[tier]+' CLASS '+(tier+1)+'</small><b>'+name+'級ミッション</b></div><span>'+(unlocked?count+'/'+missions.length:'🔒 '+name+'級で開放')+'</span></header><div class="missionGrid322">'+missions.map(m=>renderCard322(m,false)).join('')+'</div></section>';
    }).join('');
}
