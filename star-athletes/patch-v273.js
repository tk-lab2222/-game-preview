...
function tags273(m,a,b){
 const t=new Set();
 if(m?.species)t.add('species:'+m.species);
 skillIds273(m).forEach(x=>t.add('skill:'+x));
 ...
 if(a&&b){
   if(a.species===b.species)t.add('sameSpecies');else t.add('mixed');
   if(hidden273(a,'mutation')==='S'&&hidden273(b,'mutation')==='S')t.add('parents:mutationS');
   const prev=Math.max(n273(a.mixedLineage273),n273(b.mixedLineage273));m.mixedLineage273=a.species!==b.species?prev+1:0;
   if(m.mixedLineage273>=3)t.add('mixedLineage:3');
 }
 return t;
}
function evaluate273(m,a,b,roll=true){
 const tags=tags273(m,a,b),hits=[];
 for(const r of RECIPES273){
   if(!r.need.every(x=>tags.has(x)))continue;
   let mult=1;const reasons=[];for(const [tag,x] of r.mult||[])if(tags.has(tag)){mult*=x;reasons.push(`${tag}×${x}`)}
   const chance=Math.min(.25,r.base*mult),won=roll&&Math.random()<chance;
   hits.push({id:r.id,name:r.name,hint:r.hint,base:r.base,mult,chance,won,reasons});
 }
 return hits;
}
function apply273(m,a,b){
 if(!m||m.rareRecipeRolled273)return m;
 const hits=evaluate273(m,a,b,true);m.rareRecipeRolled273=true;m.rareRecipeCandidates273=hits.map(x=>({id:x.id,chance:x.chance,mult:x.mult}));
 const won=hits.filter(x=>x.won).sort((x,y)=>x.chance-y.chance)[0];
 if(won){m.specialLineage273={id:won.id,name:won.name,chance:won.chance,mult:won.mult,at:Date.now()};m.tags273=[...(m.tags273||[]),won.id]}
 return m;
}
...
function badge273(){
 document.querySelectorAll('#cands .card[data-id],#breeders .card[data-id],#lineagePool .card[data-id]').forEach(card=>{
   const id=card.dataset.id,m=[...(S.cands||[]),...(S.nest||[]),...(S.lineage||[]),...(S.starters||[])].find(x=>x?.id===id);if(!m?.specialLineage273)return;
   let b=card.querySelector('.rare273');if(!b){b=document.createElement('div');b.className='rare273';(card.querySelector('.bd')||card).appendChild(b)}
   b.textContent=`✦ ${m.specialLineage273.name} / 推定 ${(m.specialLineage273.chance*100).toFixed(m.specialLineage273.chance<.001?3:2)}%`;
 });
}
