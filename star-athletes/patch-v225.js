...
function init225(){
 ...
 S.rivals225=Array.isArray(S.rivals225)?S.rivals225:[];
}
...
function mkRivals225(promo=false){
 ...
 const lg=LEAGUES225[target],seasonBoost=((Number(S.season)||1)-1)*1.5;
 const rivals=Array.from({length:7},(_,i)=>{
   const stats={};const bias=lg.base+seasonBoost+(i-3)*1.5;
...
async function runBattle225(promo=false){
 ...
 const lg=league225(),result=document.getElementById('result');
 if(promo){
   const won=overall===1;
 ...
 const coin=Math.round((overall===1?1000:overall===2?700:overall===3?450:250)*lg.reward),fame=Math.round((overall===1?120:overall===2?80:overall===3?50:25)*lg.reward);S.coins=(S.coins||0)+coin;S.fame=(S.fame||0)+fame;
 S.seasonHistory=S.seasonHistory||[];S.seasonHistory.push({season:Number(S.season)||1,league:lg.name,name:S.seasonMeet?.name||`S${S.season}大会`,overall,points:totalPts[0],coins:coin,fame});
 if(result)result.innerHTML=`<div class="notice leagueResult225"><b>${lg.icon} ${lg.name}級・S${Number(S.season)||1}</b><br>${rows.map(x=>`${x.e}：${x.rank}位`).join('<br>')}<hr><strong>総合${overall}位 / ${totalPts[0]}pt</strong><br>🪙 +${coin}　⭐名声 +${fame}${S.promotionPending?'<br><em>🔥 昇格戦が発生！</em>':''}</div>`;
