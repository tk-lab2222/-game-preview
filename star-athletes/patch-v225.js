...
function tourChance225(t){
 const ours=teamPower225(),theirs=Math.round(league225().base+((Number(S.season)||1)-1)*1.5+t.diff),d=ours-theirs;
 const pct=Math.max(8,Math.min(92,Math.round(50+d*1.8)));
 return{ours,theirs,pct,label:pct>=72?'かなり有利':pct>=58?'やや有利':pct>=42?'互角':pct>=28?'やや不利':'強敵注意'};
}
function renderTournamentSelect225(){
 const train=document.getElementById('train');if(!train)return;
 let box=document.getElementById('tourSelect225');if(!box){box=document.createElement('div');box.id='tourSelect225';box.className='box tourSelect225';document.getElementById('league225')?.after(box);if(!box.parentNode)train.prepend(box)}
 const selected=TOURS225.find(x=>x.id===S.meetChoice225)||TOURS225[1];
 box.innerHTML=`<div class="tourHead225"><div><small>TOURNAMENT SELECT</small><b>🎯 出場大会を選ぶ</b></div><span>今季は1大会</span></div><p>成長度に合わせて、格下で安定を取るか、格上へ攻めるかを選択。</p><div class="tourChoices225">${TOURS225.map(t=>{const c=tourChance225(t);return `<button type="button" data-tour225="${t.id}" class="${selected.id===t.id?'on225':''}"><header><b>${t.icon} ${t.name}</b><strong>${c.pct}%</strong></header><small>${t.sub}</small><div><span>相手目安 ${c.theirs}</span><span>報酬 ×${t.reward}</span><span>年間pt ×${t.annual}</span></div></button>`}).join('')}</div><small class="tourNote225">※勝利見込みは自軍平均と相手目安から算出。実戦は種目適性・作戦・乱数で変動します。</small>`;
}
function renderRivals225(promo=false){
 const host=document.getElementById('rival');if(!host)return;
 const rivals=ensureRivals225(promo),c=chance225(rivals),target=LEAGUES225[Math.min(LEAGUES225.length-1,S.leagueRank+(promo?1:0))],tour=promo?null:(TOURS225.find(x=>x.id===S.meetChoice225)||TOURS225[1]);
 host.innerHTML=`<div class="rivalPanel225 ${promo?'promo225':''}"><div class="rivalTop225"><div><small>${promo?'PROMOTION BATTLE':'RIVAL SCOUT'}</small><b>${promo?'🔥 昇格戦':`${tour.icon} ${tour.name}`} / ${target.name}級</b></div><div class="chance225"><span>勝利見込み</span><strong>${c.pct}%</strong><em>${c.label}</em></div></div><div class="powerCompare225"><span>自軍平均 <b>${c.ours}</b></span><i></i><span>相手平均 <b>${c.theirs}</b></span></div><div class="rivalCards225">${rivals.slice(0,3).map(r=>`<article><header><b>${r.name}</b><em>${STAT_LABEL225[r.strong]}型</em></header><div>${STAT_KEYS225.map(k=>`<span class="${k===r.strong?'hot225':''}">${STAT_LABEL225[k]} <b>${r.stats[k]}</b></span>`).join('')}</div></article>`).join('')}</div><small class="rivalNote225">※表示されている能力値をそのまま大会計算に使用します。</small></div>`;
}
...
window.addEventListener('click',e=>{
 const b=e.target?.closest?.('[data-tour225]');if(!b||S.promotion233||S.promotionPending)return;
 e.preventDefault();const id=b.dataset.tour225;if(!TOURS225.some(x=>x.id===id))return;
 S.meetChoice225=id;S.rivals225=[];S.rivalsPromo225=false;save225();renderTournamentSelect225();renderRivals225(false);
},true);
