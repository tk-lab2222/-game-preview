...
function panel260(){
  ...
  box.innerHTML=`<div class="batchTop260"><div><small>MULTI BREED</small><b>🥚 まとめてスター配合</b><em>候補 ${Math.min(n,limit)}/${limit}</em></div><span>${phase}</span></div><div class="batchBar260"><i style="width:${Math.min(100,n/limit*100)}%"></i></div><button id="batchGo260" class="btn or" ${ready?'':'disabled'}>${n===0?`🥚 ${Math.min(3,limit)}体まとめて配合`:n>=limit?`今回の候補上限 ${limit}体`:`🥚 あと${need}体まとめて配合`}</button><div class="batchHint260">${n<3?`まず${Math.min(3,limit)}体。現在の配合上限は${limit}体です。`:n<limit?`もっと比較したい時だけ追加生成。現在の上限は${limit}体。`:`${limit}体からベスト3を選べます。`}</div>`;
...
.batchTop260{display:flex;justify-content:space-between;align-items:center;gap:8px}.batchTop260 small{display:block;font-size:6px;color:#9b7324;font-weight:1000;letter-spacing:.08em}.batchTop260 b{display:block;font-size:12px}.batchTop260 em{display:block;margin-top:2px;font-size:7px;font-style:normal;color:#8b6c2a}.batchTop260 span{font-size:7px;font-weight:1000;color:#8b681f;background:#fff1b9;border:1px solid #e3c56b;border-radius:999px;padding:3px 7px}
