(()=>{
// v0.26.0: M1.1 breeding UX. Keep selection depth, remove repetitive one-by-one hatching.
const MAX260=10;
...
function nextCount260(){const n=(S.cands||[]).length;return Math.max(0,nextTarget260(n)-n)}
function batch260(){
  ...
  const count=Math.min(nextCount260(),MAX260-(S.cands||[]).length);if(count<=0)return;
  const born=[];
...
function panel260(){
  ...
  const n=(S.cands||[]).length,need=nextCount260(),ready=Array.isArray(S.parents)&&S.parents.length===2&&!S.egg&&n<MAX260;
