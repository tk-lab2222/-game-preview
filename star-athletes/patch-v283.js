...
function render283(){
 ...
 host.innerHTML=`<div class="head283"><div><small>SUPPORT / CUSTOMIZE</small><b>🛍️ 応援・カスタマイズ</b></div><span>準備中</span></div>
 <p class="intro283">購入機能はまだありません。公開品質確認中のため、候補だけ折りたたんで表示します。</p>
 <details class="supportDetails283"><summary>候補を見る（決済なし）</summary>
 <div class="guard283">${principles283().map(x=>`<span><b>${x[0]}</b>${x[1]}</span>`).join('')}</div>
 <div class="catalog283">${cards}</div>
 <div class="bottom283"><b>設計方針</b><p>強さそのものを売らず、見た目・共有・保存・分析・限定的な補助を中心にする。価格もすべて仮置き。</p></div></details>`;
}
