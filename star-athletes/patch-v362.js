...
const COLOR={normal:1,shiny:2,gold:3,prism:4,mutation:4,divine:5};
...
function colorTier(m){if(m?.rareVisual243==='divine'||m?.visual?.color==='神彩')return'divine';if(m?.rareVisual243==='prism'||m?.visual?.color==='プリズム')return'prism';if(m?.rareVisual243==='gold'||m?.visual?.color==='金')return'gold';if(m?.rareVisual243==='mutation')return'mutation';if(m?.rareVisual243==='shiny'||m?.shiny)return'shiny';return'normal'}
function colorGrade(m){return COLOR[colorTier(m)]||1}
...
function athleteGrade(m){return Math.max(colorGrade(m),specialGrade(m),resonanceGrade(m))}
function breakdown(m){return{athlete:athleteGrade(m),color:colorGrade(m),special:specialGrade(m),resonance:resonanceGrade(m),pattern:patternGrade(m),body:bodyGrade(m),accessory:accessoryGrade(m)}}
function label(g){const z=META[clamp(g)];return z.stars+' '+z.name}
function migrate(){let changed=false;for(const m of all()){const t=colorTier(m);if(['prism','gold','divine'].includes(t)&&m.rareVisual243!==t){m.rareVisual243=t;changed=true}const g=athleteGrade(m);if(Number(m.athleteStarGrade362)!==g){m.athleteStarGrade362=g;changed=true}if('birthStarGrade361'in m){delete m.birthStarGrade361;changed=true}}if(changed)try{localStorage.setItem(SAVE,JSON.stringify({savedAt:Date.now(),S}))}catch(_){}}
...
window.STAR_GRADE_ENGINE362={meta:META,colorTier,colorGrade,specialGrade,resonanceGrade,patternGrade,bodyGrade,accessoryGrade,athleteGrade,breakdown,label,sync};
