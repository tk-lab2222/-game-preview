...
function render267(result=''){
  ...
  const e=EV267[p.type];host.innerHTML=`<div class="eventCard267"><div class="eventTitle267"><b>${e.icon} ${e.name}</b><span>${p.name}</span></div><p>${e.text}</p><div class="eventActions267"><button type="button" data-event267="a">${e.a}</button><button type="button" data-event267="b">${e.b}</button></div><small>イベント発生は運。結果はあなたの判断で変わります。</small></div>`;
