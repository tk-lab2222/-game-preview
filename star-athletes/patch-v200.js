...
function recoveryDiag200(){
  let box=document.getElementById('recoveryDiag200');
  if(!box){box=document.createElement('div');box.id='recoveryDiag200';box.className='box';box.style.cssText='border:3px solid #d33;background:#fff7f7;margin:10px';document.querySelector('.a')?.prepend(box)}
  const rows=window.STAR_SAVE_DIAG?.scan?.()||[];
  const esc=s=>String(s??'').replace(/[&<>]/g,x=>({'&':'&amp;','<':'&lt;','>':'&gt;'}[x]));
  box.innerHTML='<h3>🚑 SAVE RECOVERY DIAGNOSTIC</h3><div class="sm">読み取り専用。下の診断結果をコピーしてChatGPTへ送ってください。復旧完了まで初期化・保護保存は押さないでください。</div>'+
    '<textarea id="recoveryText200" readonly style="width:100%;height:180px;margin-top:8px;font-size:10px">'+esc(JSON.stringify(rows,null,2))+'</textarea>'+
    '<button id="copyRecovery200" class="btn" style="margin-top:7px">診断結果をコピー</button>';
  box.querySelector('#copyRecovery200').onclick=async()=>{const t=box.querySelector('#recoveryText200').value;try{await navigator.clipboard.writeText(t);box.querySelector('#copyRecovery200').textContent='✅ コピーしました'}catch(_){box.querySelector('#recoveryText200').select()}};
}
