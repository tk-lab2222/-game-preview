...
function render282(){
 ...
 box.innerHTML=`<div class="shareHead282"><div><small>MY BEST NEST</small><b>📣 ネスト代表カード</b></div><strong>${n282(b.score).toLocaleString()}</strong></div><div class="shareTeam282">${rows}</div><div class="shareMeta282"><span>SYNERGY <b>${n282(b.synergy).toLocaleString()}</b></span><span>戦術 <b>${TACTIC282[b.tactic]||b.tactic||'-'}</b></span><span>BEST <b>${new Date(b.at).toLocaleDateString('ja-JP')}</b></span></div><button type="button" id="copyShare282" class="btn yl">共有文をコピー</button><small class="shareNote282">出生レア度ではなく、代表3体・総合力・得意能力・シナジー・自己ベストを共有する競技用カードです。</small>`;
}
window.addEventListener('click',e=>{const btn=e.target?.closest?.('#copyShare282');if(btn){e.preventDefault();const b=best282();if(b)copy282(text282(b),btn);return}if(e.target?.closest?.('#register280,.tab[data-v="nest201"]'))setTimeout(render282,0)},true);
