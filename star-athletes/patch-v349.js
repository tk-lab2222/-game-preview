...
function enhanceArtStage(stage){
 ...
 if(avatar){
  avatar.classList.add('godFigure349');
  if(!avatar.parentElement.classList.contains('godFigureWrap349')){
   const wrap=document.createElement('div');wrap.className='godFigureWrap349';
...
.godFigureWrap349{position:relative;z-index:5;display:flex;align-items:flex-end;justify-content:center;width:100%;height:100%;animation:godFloat349 3.2s ease-in-out infinite}
.godFigure349{position:relative!important;z-index:6!important;transform:scale(1.18);filter:drop-shadow(0 0 8px rgba(255,255,255,.95)) drop-shadow(0 0 18px rgba(255,226,107,.75)) drop-shadow(0 0 30px rgba(169,120,255,.65))}
...
@keyframes godFloat349{0%,100%{transform:translateY(0)}50%{transform:translateY(-6px)}}
