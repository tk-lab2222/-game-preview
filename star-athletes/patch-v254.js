...
function closeSkillTip254(){document.getElementById('skillTip254')?.remove()}
function showSkillTip254(el,id){
 const s=SK254[id];if(!s)return;closeSkillTip254();
 const tip=document.createElement('div');tip.id='skillTip254';tip.className='skillTip254';
 tip.innerHTML='<b>'+s.icon+' '+s.name+'</b><span>'+s.desc+'</span>';
 document.body.appendChild(tip);
 const r=el.getBoundingClientRect(),pad=8,w=Math.min(260,window.innerWidth-pad*2);
 let left=Math.max(pad,Math.min(window.innerWidth-w-pad,r.left+r.width/2-w/2));
 let top=r.bottom+7;if(top+70>window.innerHeight)top=Math.max(pad,r.top-70);
 tip.style.width=w+'px';tip.style.left=left+'px';tip.style.top=top+'px';
}
function installSkillTap254(){
 if(window.__SKILL_TAP254)return;window.__SKILL_TAP254=true;
 document.addEventListener('click',e=>{const el=e.target.closest?.('[data-skill254]');if(el){e.preventDefault();e.stopPropagation();showSkillTip254(el,el.dataset.skill254);return}if(!e.target.closest?.('#skillTip254'))closeSkillTip254()},true);
 window.addEventListener('scroll',closeSkillTip254,{passive:true,capture:true});
 window.addEventListener('touchmove',closeSkillTip254,{passive:true,capture:true});
}
...
function sync254(){try{installSkillTap254();cardSkills254();compat254();hatchSkill254()}catch(e){console.warn('sync254',e)}}
