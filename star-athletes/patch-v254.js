...
function cardSkills254(){
 document.querySelectorAll('#breeders .card[data-id],#cands .card[data-id]').forEach(card=>{
   const m=byId254(card.dataset.id);if(!m)return;const host=card.querySelector('.bd')||card;
   let row=host.querySelector(':scope>.breedSkill254');if(!row){row=document.createElement('div');row.className='breedSkill254';host.appendChild(row)}
   row.innerHTML=`<span class="breedSkillLabel254">SKILL</span><div>${skillBadges254(m)}</div>`;
 });
 // This patch owns the visible skill row on breeding/candidate cards to avoid duplicate/vanishing late rows.
