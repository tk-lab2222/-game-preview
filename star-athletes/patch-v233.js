...
function finishGen233(){if(!S.promotion233&&!S.planetRepresentative233&&n233(S.season)<6){console.warn('blocked premature finishGen233',S.season,S.leagueRank);return advanceSeason233()}try{window.STAR_GRADE340?.finalizeAbilityRanks?.(S.nest||[])}catch(e){console.warn('ability rank finalize',e)}S.needsBreeding=true;S.generationActive=false;S.parents=[];S.cands=[];S.sel=[];S.egg=null;S.turn=0;S.schedule=[];S.plans={};S.assign={};S.strat={};S.season=1;S.seasonMeet=null;S.rivals225=[];S.rivalsPromo225=false;S.promotionPending=false;S.meetChoice225='standard';S.promotion233=false;S.generation233=(n233(S.generation233)||1)+1;S.annual233=null;save233();const u=new URL(location.href);u.search='';u.searchParams.delete('v');u.searchParams.set('breed','1');u.searchParams.set('t',Date.now());location.replace(u.toString())}
function nextGen233(label='🥚 次世代配合へ'){if(!S.promotion233&&!S.planetRepresentative233&&n233(S.season)<6){return advanceSeason233()}const p=document.querySelector('#meet .box p');if(!p)return;document.getElementById('next225')?.style.setProperty('display','none','important');let b=document.getElementById('annualNext233');if(!b){b=document.createElement('button');b.id='annualNext233';b.className='btn yl';b.type='button';p.appendChild(b)}b.textContent=label;b.onclick=finishGen233;b.style.display='inline-block'}
