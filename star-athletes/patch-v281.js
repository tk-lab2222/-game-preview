(()=>{
// v0.29.4 / M8: optional continuity board.
// No login streak, no exclusive power. Daily/weekly rewards are small and only create a reason to return.
const SAVE281='star-athletes-save-v200';
const DAY=86400000,WEEK=7*DAY,SEASON=28*DAY;
function n281(v){return Number(v)||0}
function jstNow281(){return Date.now()+9*60*60*1000}
function dayKey281(){return Math.floor(jstNow281()/DAY)}
function weekKey281(){return Math.floor(jstNow281()/WEEK)}
function seasonKey281(){return Math.floor(jstNow281()/SEASON)}
function gen281(){return Math.max(1,n281(S.nest?.[0]?.gen)||n281(S.lineage?.[0]?.gen)||1)}
