...
const P={
 ...
 wing_left:{normal:[-.6,.4,1,-8,1],feather:[-.6,.4,1,-8,1],crystal:[-.4,.2,.98,-7,1]},
 wing_right:{normal:[.4,0,.98,7,0],feather:[.4,0,.98,7,0],crystal:[.4,-.2,.96,6,0]},
...
async function wingless(){if(winglessPromise)return winglessPromise;winglessPromise=(async()=>{const [body,wing]=await Promise.all([img(G+'body_stage.png?v=411'),img(G+'wing_normal_clean.png?v=411')]);const c=document.createElement('canvas');c.width=W;c.height=H;const x=c.getContext('2d');x.drawImage(body,0,0,W,H);x.globalCompositeOperation='destination-out';drawPart(x,wing,'wing_left','normal',1.045);drawPart(x,wing,'wing_right','normal',1.045);x.globalCompositeOperation='source-over';return c})();return winglessPromise}
async function paintOne(c){if(c.dataset.painting==='1')return;c.dataset.painting='1';try{const horn=c.dataset.horn||'normal',wing=c.dataset.wing||'normal',tail=c.dataset.tail||'normal';c.width=W;c.height=H;const ctx=c.getContext('2d');ctx.clearRect(0,0,W,H);const [base,wL,wR]=await Promise.all([wingless(),img(G+`wing_${wing}_clean.png?v=411`),img(G+`wing_${wing}_clean.png?v=411`)]);drawPart(ctx,wL,'wing_left',wing);drawPart(ctx,wR,'wing_right',wing);ctx.drawImage(base,0,0,W,H);if(tail!=='normal'){const t=await img(G+`tail_${tail}_clean.png?v=411`);drawPart(ctx,t,'tail',tail)}if(horn!=='normal'){const [hl,hr]=await Promise.all([img(G+`horn_${horn}_left.png?v=413`),img(G+`horn_${horn}_right.png?v=413`)]);drawPart(ctx,hl,'horn_left',horn);drawPart(ctx,hr,'horn_right',horn)}c.dataset.painting='0';c.dataset.painted='1'}catch(e){c.dataset.painting='0'}}
function paintAll(){document.querySelectorAll('canvas.dracoCanvas').forEach(paintOne)}
const style=document.createElement('style');style.textContent='.avatar.draco-live{position:relative;overflow:hidden;background:linear-gradient(#fff9eb,#f4ead6);padding:0}.dracoCanvas{width:100%;height:100%;display:block;object-fit:contain}.geneMini{position:absolute;left:4px;bottom:3px;z-index:8;background:#fffdf2dd;border:1px solid #222;border-radius:999px;padding:1px 4px;font-size:6px;font-weight:900}.avatar.draco-live .pattern,.avatar.draco-live .accessory{display:none}';document.head.appendChild(style);
