...
function register323(m){
 if(!m)return;
 S.myStar323={athlete:clone323(m),registeredAt:Date.now(),sourceId:m.id};
...
function current323(){return S.myStar323?.athlete||null}
...
function render323(){
 const sec=ensureSection323();if(!sec)return;const m=current323();
 if(!m){sec.innerHTML='<div class="myStarTitle323"><small>HALL OF FAME</small><h2>⭐ MY STAR</h2><p>自分史上最高の1体を、能力・スキル・称号ごと永久保存する殿堂。</p></div><div class="myStarEmpty323"><b>まだMY STARは登録されていません</b><span>個体カードの「★ MY STAR」から、渾身の1体を登録できます。</span></div>';return}
 const max=Math.max(...Object.values(m.stats||{}).map(n323),0),titles=titleBadges323(m);
 sec.innerHTML='<div class="myStarTitle323"><small>HALL OF FAME</small><h2>⭐ MY STAR</h2><p>代表でも親候補でもない、あなたが選んだ「渾身の1体」。</p></div>'+
 '<article class="myStarHero323">'+
 '<div class="myStarAura323"></div><header><div class="myStarAvatar323">'+(typeof avatar==='function'?avatar(m):'')+'</div><div class="myStarIdentity323"><small>MY STAR / G'+(n323(m.gen)||n323(m.generation)||0)+'</small><h3>'+m.name+'</h3><b>'+(typeof SP!=='undefined'&&SP[m.species]?SP[m.species][0]:m.species||'')+' ・ '+(m.rarity||'C')+'</b><em>'+(m.personality||'')+'</em><strong>最高能力 '+max+'</strong></div></header>'+
 ...
 '<footer>登録：'+new Date(S.myStar323.registeredAt||Date.now()).toLocaleDateString('ja-JP')+'<button type="button" data-open-source323="'+(S.myStar323.sourceId||'')+'">現在の個体データで更新</button></footer>'+
 '</article>';
...
function addButton323(card,m){
 ...
 const b=document.createElement('button');b.type='button';b.className='myStarBtn323';b.dataset.myStar323=m.id;b.textContent=S.myStar323?.sourceId===m.id?'★ MY STAR 登録中':'☆ MY STAR';
...
function syncButtons323(){
 ...
 document.querySelectorAll('.myStarBtn323').forEach(b=>b.classList.toggle('on323',S.myStar323?.sourceId===b.dataset.myStar323));
}
...
document.addEventListener('click',e=>{
 ...
 const upd=e.target?.closest?.('[data-open-source323]');if(upd){const m=find323(upd.dataset.openSource323);if(m)register323(m)}
},true);
try{const prev=render;render=function(){const out=prev();setTimeout(sync323,0);return out}}catch(e){console.warn('my star 323',e)}
window.STAR_MY_STAR323={sync:sync323,register:register323,current:current323,show:showHall323};
const css=document.createElement('style');css.textContent=`
#hall323{padding-bottom:90px}.myStarTitle323{margin:8px 0 10px}.myStarTitle323 small{font-size:7px;letter-spacing:.16em;color:#7960a3;font-weight:1000}.myStarTitle323 h2{margin:2px 0}.myStarTitle323 p{font-size:8px;color:#687589}.myStarEmpty323{padding:28px 14px;border:2px dashed #b8a8cc;border-radius:16px;background:#faf7ff;text-align:center}.myStarEmpty323 b,.myStarEmpty323 span{display:block}.myStarEmpty323 span{font-size:8px;color:#758091;margin-top:5px}
