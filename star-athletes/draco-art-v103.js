...
const style=document.createElement('style');style.textContent='.avatar.draco-live{position:relative;overflow:hidden;background:linear-gradient(#fff9eb,#f4ead6);padding:0}.dracoCanvas{position:absolute;inset:4%;width:92%;height:92%;display:block}.train210>header .avatar.draco-live .dracoCanvas,.entries210 .avatar.draco-live .dracoCanvas{inset:7%;width:86%;height:86%}.geneMini{position:absolute;left:4px;bottom:3px;z-index:8;background:#fffdf2dd;border:1px solid #222;border-radius:999px;padding:1px 4px;font-size:6px;font-weight:900}.avatar.draco-live .pattern,.avatar.draco-live .accessory{display:none}';document.head.appendChild(style);
