(()=>{
// v0.20.6: recalc affinity from the actual parent slots and add a visible egg-crack beat before hatch.
...
function setupCrack206(){
  ...
  hatch.addEventListener('click',()=>{
    ...
    egg.classList.remove('cracking206','split206');
    void egg.offsetWidth;
    egg.classList.add('cracking206');
    setTimeout(()=>egg.classList.add('split206'),380);
    setTimeout(()=>egg.classList.remove('cracking206','split206'),980);
  },true);
}
const before206=render;
render=function(){const out=before206();drawAffinity206();setupCrack206();return out};
const css=document.createElement('style');css.textContent=`
/* Affinity gets a stronger visible state once both parents are chosen. */
...
/* Egg crack: keep the existing 850ms hatch delay, but make that delay visually meaningful. */
...
.cracking206 .eggShell127:before{opacity:1;height:24px;transform:translateX(-7px) rotate(24deg);animation:crackGrow206 .22s ease-out forwards}.cracking206 .eggShell127:after{opacity:1;height:19px;transform:translateX(7px) translateY(17px) rotate(-30deg);animation:crackGrow206 .2s .12s ease-out both}
.cracking206 .eggShell127 span{animation:eggShake206 .12s linear infinite!important}.split206 .eggShell127 span{animation:eggOpen206 .44s cubic-bezier(.2,.8,.2,1) forwards!important}.split206 .eggAura127{animation:flash206 .44s ease-out forwards!important}
@keyframes crackGrow206{from{height:0;opacity:0}to{opacity:1}}
@keyframes eggShake206{25%{transform:translateX(-4px) rotate(-4deg)}75%{transform:translateX(4px) rotate(4deg)}}
@keyframes eggOpen206{0%{transform:scale(1);filter:drop-shadow(0 0 10px var(--eggB,#ffd78a))}45%{transform:scale(1.08)}70%{transform:scaleX(1.18) scaleY(.9);opacity:1}100%{transform:scale(1.45);opacity:0;filter:brightness(2)}}
@keyframes flash206{0%{opacity:.45;transform:scale(1)}60%{opacity:1;transform:scale(1.55)}100%{opacity:0;transform:scale(2.3)}}
`;
document.head.appendChild(css);
setTimeout(()=>{try{observeParents206();drawAffinity206();setupCrack206()}catch(e){console.error('v0.20.6',e)}},0);
})();