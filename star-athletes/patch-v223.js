...
function cap223(){try{return Math.max(999,Number(window.STAR_LIMIT278?.cap?.())||999)}catch(_){return 999}}
function highStatMul223(v){
 const cap=cap223();
 if(cap<=999)return v>=950?.4:v>=850?.65:v>=700?.85:1;
 const r=(Number(v)||0)/cap;
 return r>=.90?.45:r>=.75?.62:r>=.60?.78:r>=.45?.90:1;
}
