...
function clean357(){
 ...
 // v127/v201 legacy badge beside newborn name.
 const name=box.querySelector('.hatchName');if(name){
  [...name.querySelectorAll('span')].forEach(el=>{if(/^(C|U|R|SR|SSR|UR|EX)$/.test((el.textContent||'').trim()))el.remove()});
 }
...
#birth .hatchName>span:not(.keepHatch357){display:none!important}
