(async()=>{try{
  ...
  try{
    ...
    if(r.ok){
      ...
      if(latest&&latest!==CURRENT){
        ...
        if(u.searchParams.get('v')!==latest){
  ...
  const patch=`
<style id="v062-board-layout-fix">
  ...
  #gameVersionBadge{position:fixed;right:7px;top:calc(env(safe-area-inset-top,0px) + 5px);z-index:9999;padding:3px 7px;border-radius:999px;background:rgba(5,10,30,.76);border:1px solid rgba(255,255,255,.2);font:700 10px/1.2 system-ui,-apple-system,sans-serif;color:#fff;letter-spacing:.04em;pointer-events:none;box-shadow:0 2px 10px rgba(0,0,0,.25)}
...
<div id="gameVersionBadge">v0.6.2</div>
<script id="v062-board-layout-script">
(()=>{
...
<\/script>`;
  txt=txt.replace(/<title>[^<]*<\/title>/,'<title>星火ピンボール v0.6.2 決戦</title>');
